...
void xfade_tilde_dsp(t_xfade_tilde* x, t_signal** sp)
{
     ...
     */
    dsp_add(xfade_tilde_perform, 5, x, sp[0]->s_vec, sp[1]->s_vec,
            sp[2]->s_vec, sp[0]->s_n);
}
...
{
    xfade_tilde_class = class_new(
        gensym("xfade~"), (t_newmethod)xfade_tilde_new,
        (t_method)xfade_tilde_free, sizeof(t_xfade_tilde), CLASS_DEFAULT,
        A_DEFFLOAT, 0);

    /* whenever the audio-engine is turned on, the "xfade_tilde_dsp()"
     * function will get called
     */
    class_addmethod(xfade_tilde_class, (t_method)xfade_tilde_dsp,
                    gensym("dsp"), A_CANT, 0);
