...
void multy_dsp64(t_multy* x, t_object* dsp64, short* count, double samplerate,
                 long maxvectorsize, long flags);
void multy_perform64(t_multy* x, t_object* dsp64, double** ins, long numins,
                     double** outs, long numouts, long vectorsize, long flags,
                     void* userparams);
...
int C74_EXPORT main()
{
    ...
    multy_class = class_new("multy~", (method)multy_new, (method)multy_free,
...
void multy_assist(t_multy* x, void* b, long msg, long arg, char* dst)
{
    ...
    if (msg == ASSIST_INLET) {
        switch (arg) {
        case I_INPUT1:
            snprintf_zero(dst, ASSIST_MAX_STRING_LEN, "(signal) Input 1");
            break;
        case I_INPUT2:
            snprintf_zero(dst, ASSIST_MAX_STRING_LEN, "(signal) Input 2");
            break;
        }
    }
                 ...
                 long maxvectorsize, long flags)
{
    ...
    // dsp_add64(dsp64, (t_object *)x, (t_perfroutine64)multy_perform64, 0,
    // NULL);
 ...
 * ****************************************************/
void multy_perform64(t_multy* x, t_object* dsp64, double** inputs,
                     long numinputs, double** outputs, long numoutputs,
