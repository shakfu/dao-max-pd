...
void scrubber_dsp64(t_scrubber* x, t_object* dsp64, short* count,
                    double samplerate, long maxvectorsize, long flags);
void scrubber_perform64(t_scrubber* x, t_object* dsp64, double** ins,
...
int C74_EXPORT main()
{
    ...
    scrubber_class = class_new("scrubber~", (method)scrubber_new,
                               (method)scrubber_free, sizeof(t_scrubber), 0,
                               A_GIMME, 0);

    /* Bind the DSP method, which is called when the DACs are turned on */
    class_addmethod(scrubber_class, (method)scrubber_dsp64, "dsp64", A_CANT,
                    0);

    /* Bind the float method, which is called when floats are sent to inlets */
    class_addmethod(scrubber_class, (method)scrubber_float, "float", A_FLOAT,
                    0);

    /* Bind the assist method, which is called on mouse-overs to inlets and
     * outlets */
    class_addmethod(scrubber_class, (method)scrubber_assist, "assist", A_CANT,
                    0);

    /* Bind the object-specific methods */
    class_addmethod(scrubber_class, (method)scrubber_sample, "sample", 0);
    class_addmethod(scrubber_class, (method)scrubber_overlap, "overlap",
                    A_FLOAT, 0);
    class_addmethod(scrubber_class, (method)scrubber_resize, "resize", A_FLOAT,
                    0);
...
void scrubber_assist(t_scrubber* x, void* b, long msg, long arg, char* dst)
{
    ...
    if (msg == ASSIST_INLET) {
        switch (arg) {
        case I_REAL:
            snprintf_zero(dst, ASSIST_MAX_STRING_LEN, "(signal) Real part");
            break;
        case I_IMAG:
            ...
            break;
        case I_SPEED:
            snprintf_zero(dst, ASSIST_MAX_STRING_LEN, "(signal) Speed");
            break;
        case I_POSITION:
            snprintf_zero(dst, ASSIST_MAX_STRING_LEN, "(signal) Position");
            break;
        }
    }
