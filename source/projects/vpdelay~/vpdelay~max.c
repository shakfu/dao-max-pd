 ...
 * ************************************************************/
#define IS_DENORM(v)                                                          \
    ((((*(unsigned long*)&(v)) & 0x7f800000) == 0) && ((v) != 0.f))
...
void vpdelay_dsp64(t_vpdelay* x, t_object* dsp64, short* count,
                   double samplerate, long maxvectorsize, long flags);
void vpdelay_perform64(t_vpdelay* x, t_object* dsp64, double** ins,
...
int C74_EXPORT main()
{
    ...
    vpdelay_class = class_new("vpdelay~", (method)vpdelay_new,
                              (method)vpdelay_free, sizeof(t_vpdelay), 0,
                              A_GIMME, 0);
     ...
     * outlets */
    class_addmethod(vpdelay_class, (method)vpdelay_assist, "assist", A_CANT,
                    0);
...
void* vpdelay_common_new(t_vpdelay* x, short argc, t_atom* argv)
{
    ...
    if (max_delay < MINIMUM_MAX_DELAY) {
        ...
        post("vpdelay~ • Invalid argument: Maximum delay time set to %.4f[ms]",
             max_delay);
    ...
    if (feedback < MINIMUM_FEEDBACK) {
        ...
        post("vpdelay~ • Invalid argument: Feedback factor set to %.4f",
             feedback);
