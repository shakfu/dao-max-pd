...
void vpdelay_tilde_setup(void)
{
    ...
    vpdelay_class = class_new(gensym("vpdelay~"), (t_newmethod)vpdelay_new,
                              (t_method)vpdelay_free, sizeof(t_vpdelay), 0,
                              A_GIMME, 0);
...
void vpdelay_dsp(t_vpdelay* x, t_signal** sp, short* count)
{
    ...
    dsp_add(vpdelay_perform, NEXT - 1, x, sp[0]->s_vec, sp[1]->s_vec,
