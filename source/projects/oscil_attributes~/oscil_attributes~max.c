...
/* Function prototypes
 * ********************************************************/
void* oscil_attributes_common_new(t_oscil_attributes* x, short argc,
                                  t_atom* argv);
void oscil_attributes_free(t_oscil_attributes* x);
void oscil_attributes_dsp64(t_oscil_attributes* x, t_object* dsp64,
                            short* count, double samplerate,
                            long maxvectorsize, long flags);
void oscil_attributes_perform64(t_oscil_attributes* x, t_object* dsp64,
...
void oscil_attributes_build_list(t_oscil_attributes* x, t_symbol* msg,
                                 short argc, t_atom* argv);
void oscil_attributes_build_waveform(t_oscil_attributes* x);
void oscil_attributes_fadetime(t_oscil_attributes* x, t_symbol* msg,
                               short argc, t_atom* argv);
void oscil_attributes_fadetype(t_oscil_attributes* x, t_symbol* msg,
                               short argc, t_atom* argv);
...
t_max_err a_frequency_set(t_oscil_attributes* x, void* attr, long ac,
                          t_atom* av);
t_max_err a_crossfade_type_set(t_oscil_attributes* x, void* attr, long ac,
                               t_atom* av);
t_max_err a_crossfade_time_set(t_oscil_attributes* x, void* attr, long ac,
                               t_atom* av);
t_max_err a_waveform_set(t_oscil_attributes* x, void* attr, long ac,
                         t_atom* av);
t_max_err a_harmonics_set(t_oscil_attributes* x, void* attr, long ac,
                          t_atom* av);
t_max_err a_amplitudes_get(t_oscil_attributes* x, void* attr, long* ac,
                           t_atom** av);
t_max_err a_amplitudes_set(t_oscil_attributes* x, void* attr, long ac,
                           t_atom* av);
void oscil_attributes_assist(t_oscil_attributes* x, void* b, long msg,
                             long arg, char* dst);
...
int C74_EXPORT main()
{
    ...
    oscil_attributes_class = class_new(
        "oscil_attributes~", (method)oscil_attributes_new,
        (method)oscil_attributes_free, sizeof(t_oscil_attributes), 0, A_GIMME,
        0);

    /* Bind the DSP method, which is called when the DACs are turned on */
    class_addmethod(oscil_attributes_class, (method)oscil_attributes_dsp64,
                    "dsp64", A_CANT, 0);

    /* Bind the float method, which is called when floats are sent to inlets */
    class_addmethod(oscil_attributes_class, (method)oscil_attributes_float,
                    "float", A_FLOAT, 0);

    /* Bind the assist method, which is called on mouse-overs to inlets and
     * outlets */
    class_addmethod(oscil_attributes_class, (method)oscil_attributes_assist,
                    "assist", A_CANT, 0);

    /* Bind the object-specific methods */
    class_addmethod(oscil_attributes_class,
                    (method)oscil_attributes_build_sine, "sine", 0);
    class_addmethod(oscil_attributes_class,
                    (method)oscil_attributes_build_triangle, "triangle", 0);
    class_addmethod(oscil_attributes_class,
                    (method)oscil_attributes_build_sawtooth, "sawtooth", 0);
    class_addmethod(oscil_attributes_class,
                    (method)oscil_attributes_build_square, "square", 0);
    class_addmethod(oscil_attributes_class,
                    (method)oscil_attributes_build_pulse, "pulse", 0);
    class_addmethod(oscil_attributes_class,
                    (method)oscil_attributes_build_list, "list", A_GIMME, 0);
    class_addmethod(oscil_attributes_class, (method)oscil_attributes_fadetime,
                    "fadetime", A_GIMME, 0);
    class_addmethod(oscil_attributes_class, (method)oscil_attributes_fadetype,
                    "fadetype", A_GIMME, 0);
    ...
    /* Bind the attributes */
    CLASS_ATTR_FLOAT(oscil_attributes_class, "frequency", 0,
                     t_oscil_attributes, a_frequency);
    CLASS_ATTR_LABEL(oscil_attributes_class, "frequency", 0, "Frequency");
    CLASS_ATTR_ORDER(oscil_attributes_class, "frequency", 0, "1");
    CLASS_ATTR_ACCESSORS(oscil_attributes_class, "frequency", NULL,
                         a_frequency_set);

    CLASS_ATTR_LONG(oscil_attributes_class, "fadetype", 0, t_oscil_attributes,
                    a_crossfade_type);
    ...
    CLASS_ATTR_ACCESSORS(oscil_attributes_class, "fadetype", NULL,
                         a_crossfade_type_set);

    CLASS_ATTR_FLOAT(oscil_attributes_class, "fadetime", 0, t_oscil_attributes,
                     a_crossfade_time);
    CLASS_ATTR_LABEL(oscil_attributes_class, "fadetime", 0, "Crossfade time");
    CLASS_ATTR_ORDER(oscil_attributes_class, "fadetime", 0, "3");
    CLASS_ATTR_ACCESSORS(oscil_attributes_class, "fadetime", NULL,
                         a_crossfade_time_set);

    CLASS_ATTR_SYM(oscil_attributes_class, "waveform", 0, t_oscil_attributes,
                   a_waveform);
    CLASS_ATTR_LABEL(oscil_attributes_class, "waveform", 0, "Waveform");
    CLASS_ATTR_ORDER(oscil_attributes_class, "waveform", 0, "4");
    CLASS_ATTR_ENUM(oscil_attributes_class, "waveform", 0,
                    "sine triangle sawtooth square pulse additive");
    CLASS_ATTR_ACCESSORS(oscil_attributes_class, "waveform", NULL,
                         a_waveform_set);

    CLASS_ATTR_LONG(oscil_attributes_class, "harmonics", 0, t_oscil_attributes,
                    a_harmonics);
    CLASS_ATTR_LABEL(oscil_attributes_class, "harmonics", 0, "Harmonics");
    CLASS_ATTR_ORDER(oscil_attributes_class, "harmonics", 0, "5");
    CLASS_ATTR_ACCESSORS(oscil_attributes_class, "harmonics", NULL,
                         a_harmonics_set);

    CLASS_ATTR_FLOAT_ARRAY(oscil_attributes_class, "amplitudes", 0,
                           t_oscil_attributes, a_amplitudes,
                           MAXIMUM_HARMONICS);
    CLASS_ATTR_LABEL(oscil_attributes_class, "amplitudes", 0, "Amplitudes");
    CLASS_ATTR_ORDER(oscil_attributes_class, "amplitudes", 0, "6");
    CLASS_ATTR_ACCESSORS(oscil_attributes_class, "amplitudes",
                         a_amplitudes_get, a_amplitudes_set);
...
void* oscil_attributes_new(t_symbol* s, short argc, t_atom* argv)
{
    ...
    t_oscil_attributes* x = (t_oscil_attributes*)object_alloc(
        oscil_attributes_class);
                           ...
                           t_atom* av)
{
    ...
    t_atom* rv = NULL;
    object_method_sym((t_object*)x, gensym("waveform"), gensym("additive"),
                      rv);
...
void oscil_attributes_build_wavetable(t_oscil_attributes* x)
{
    if (x->waveform == gensym("sine")) {
        ...
        error("oscil_attributes~ • Invalid argument: Waveform set to %s",
              x->waveform->s_name);
    }
}
                            ...
                            long maxvectorsize, long flags)
{
    ...
    object_method(dsp64, gensym("dsp_add64"), x, oscil_attributes_perform64, 0,
                  NULL);
                                ...
                                void* userparam)
{
    ...
    while (n--) {
        ...
        if (x->dirty) {
            ...
            fraction = (t_double)crossfade_countdown
                / (t_double)crossfade_samples;
