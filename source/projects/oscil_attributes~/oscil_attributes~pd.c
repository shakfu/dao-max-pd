...
void oscil_attributes_tilde_setup(void)
{
    ...
    oscil_attributes_class = class_new(
        gensym("oscil_attributes~"), (t_newmethod)oscil_attributes_new,
        (t_method)oscil_attributes_free, sizeof(t_oscil_attributes), 0,
        A_GIMME, 0);
    ...
    class_addmethod(oscil_attributes_class, (t_method)oscil_attributes_dsp,
                    gensym("dsp"), 0);

    /* Bind the object-specific methods */
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_build_sine, gensym("sine"), 0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_build_triangle,
                    gensym("triangle"), 0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_build_sawtooth,
                    gensym("sawtooth"), 0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_build_square, gensym("square"),
                    0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_build_pulse, gensym("pulse"),
                    0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_build_list, gensym("list"),
                    A_GIMME, 0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_fadetime, gensym("fadetime"),
                    A_GIMME, 0);
    class_addmethod(oscil_attributes_class,
                    (t_method)oscil_attributes_fadetype, gensym("fadetype"),
                    A_GIMME, 0);
...
void* oscil_attributes_new(t_symbol* s, short argc, t_atom* argv)
{
    ...
    t_oscil_attributes* x = (t_oscil_attributes*)pd_new(
...
void* new_memory(long nbytes)
{
    ...
    if (pointer == NULL) {
        pd_error(NULL,
                 "oscil_attributes~ • Cannot allocate memory for this object");
        return NULL;
    }
    return pointer;
}
...
void oscil_attributes_build_wavetable(t_oscil_attributes* x)
{
    if (x->waveform == gensym("sine")) {
        ...
        pd_error(x, "oscil_attributes~ • Invalid argument: Waveform set to %s",
                 x->waveform->s_name);
    }
}
...
void oscil_attributes_dsp(t_oscil_attributes* x, t_signal** sp, short* count)
{
    ...
    dsp_add(oscil_attributes_perform, NEXT - 1, x, sp[0]->s_vec, sp[1]->s_vec,
            sp[0]->s_n);
