...
void retroseq_tilde_setup(void)
{
    ...
    retroseq_class = class_new(gensym("retroseq~"), (t_newmethod)retroseq_new,
                               (t_method)retroseq_free, sizeof(t_retroseq), 0,
                               A_GIMME, 0);
    ...
    class_addmethod(retroseq_class, (t_method)retroseq_list, gensym("list"),
                    A_GIMME, 0);
    class_addmethod(retroseq_class, (t_method)retroseq_freqlist,
                    gensym("freqlist"), A_GIMME, 0);
    class_addmethod(retroseq_class, (t_method)retroseq_durlist,
                    gensym("durlist"), A_GIMME, 0);

    class_addmethod(retroseq_class, (t_method)retroseq_shuffle_freqs,
                    gensym("shuffle_freqs"), 0);
    class_addmethod(retroseq_class, (t_method)retroseq_shuffle_durs,
                    gensym("shuffle_durs"), 0);
    class_addmethod(retroseq_class, (t_method)retroseq_shuffle,
                    gensym("shuffle"), 0);

    class_addmethod(retroseq_class, (t_method)retroseq_set_tempo,
                    gensym("tempo"), A_GIMME, 0);
    class_addmethod(retroseq_class, (t_method)retroseq_set_elastic_sustain,
                    gensym("elastic_sustain"), A_GIMME, 0);
    class_addmethod(retroseq_class, (t_method)retroseq_set_sustain_amplitude,
                    gensym("sustain_amplitude"), A_GIMME, 0);
    class_addmethod(retroseq_class, (t_method)retroseq_set_adsr,
                    gensym("adsr"), A_GIMME, 0);

    class_addmethod(retroseq_class, (t_method)retroseq_manual_override,
                    gensym("manual_override"), A_GIMME, 0);
    class_addmethod(retroseq_class, (t_method)retroseq_trigger_sent,
                    gensym("bang"), 0);
    class_addmethod(retroseq_class, (t_method)retroseq_play_backwards,
                    gensym("play_backwards"), A_GIMME, 0);
                      ...
                      int arg_index, short argc, t_atom* argv)
{
    *variable = default_value;
    ...
    if (argc > arg_index) {
        *variable = atom_getsymbolarg(arg_index, argc, argv);
    }
}
...
void free_memory(void* ptr, long nbytes) { freebytes(ptr, nbytes); }
...
void retroseq_list(t_retroseq* x, t_symbol* msg, short argc, t_atom* argv)
{
    ...
    if (argc % 2) {
        pd_error(x,
                 "retroseq~ • The sequence must have odd number of members");
        return;
    }
...
void retroseq_dsp(t_retroseq* x, t_signal** sp, short* count)
{
    ...
    dsp_add(retroseq_perform, NEXT - 1, x, sp[0]->s_vec, sp[0]->s_n);
