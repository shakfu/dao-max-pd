...
void oscil_tilde_setup(void)
{
    ...
    oscil_class = class_new(gensym("oscil~"), (t_newmethod)oscil_new,
                            (t_method)oscil_free, sizeof(t_oscil), 0, A_GIMME,
                            0);
    ...
    class_addmethod(oscil_class, (t_method)oscil_build_sine, gensym("sine"),
                    0);
    class_addmethod(oscil_class, (t_method)oscil_build_triangle,
                    gensym("triangle"), 0);
    class_addmethod(oscil_class, (t_method)oscil_build_sawtooth,
                    gensym("sawtooth"), 0);
    class_addmethod(oscil_class, (t_method)oscil_build_square,
                    gensym("square"), 0);
    class_addmethod(oscil_class, (t_method)oscil_build_pulse, gensym("pulse"),
                    0);
    class_addmethod(oscil_class, (t_method)oscil_build_list, gensym("list"),
                    A_GIMME, 0);
    class_addmethod(oscil_class, (t_method)oscil_fadetime, gensym("fadetime"),
                    A_GIMME, 0);
    class_addmethod(oscil_class, (t_method)oscil_fadetype, gensym("fadetype"),
                    A_GIMME, 0);
...
void* oscil_common_new(t_oscil* x, short argc, t_atom* argv)
{
    ...
    if (x->waveform == gensym("sine")) {
        ...
        pd_error(x, "oscil~ • Invalid argument: Waveform set to %s",
                 x->waveform->s_name);
    }
...
void oscil_dsp(t_oscil* x, t_signal** sp, short* count)
{
    ...
    dsp_add(oscil_perform, NEXT - 1, x, sp[0]->s_vec, sp[1]->s_vec,
            sp[0]->s_n);
