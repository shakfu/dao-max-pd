...
void windowvec_dsp64(t_windowvec* x, t_object* dsp64, short* count,
                     double samplerate, long maxvectorsize, long flags);
void windowvec_perform64(t_windowvec* x, t_object* dsp64, double** ins,
...
int C74_EXPORT main()
{
    ...
    windowvec_class = class_new("windowvec~", (method)windowvec_new,
                                (method)windowvec_free, sizeof(t_windowvec), 0,
                                A_GIMME, 0);

    /* Bind the DSP method, which is called when the DACs are turned on */
    class_addmethod(windowvec_class, (method)windowvec_dsp64, "dsp64", A_CANT,
                    0);

    /* Bind the float method, which is called when floats are sent to inlets */
    class_addmethod(windowvec_class, (method)windowvec_float, "float", A_FLOAT,
                    0);

    /* Bind the assist method, which is called on mouse-overs to inlets and
     * outlets */
    class_addmethod(windowvec_class, (method)windowvec_assist, "assist",
                    A_CANT, 0);
